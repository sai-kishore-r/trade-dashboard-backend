...
router.post("/sync-52week-marketbreath", async (req, res) => {
  ...
  try {
    ...
    const latestRecords = await dbWrapper.getAllMarketBreadth(); // Returns all sorted descending by date
    ...
    if (latestSyncedDateStr) {
      ...
      // No data in DB, sync full past year
    ...
    // Batch upsert all dateMap entries
